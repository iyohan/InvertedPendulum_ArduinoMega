...
double voltage = 0;
float pitch_dir = 0;  // change name later.
float u1 = 0;
float u2 = 0;
float u3 = 0;
float u4 = 0;
...
void loop() {
  ...
  if (time_curr - control_time_prev >= INTERVAL_CONTROL) {
    ...
    const float PITCH_OFFSET = -1;  // -1 is default due to the error of the system built.
    
    u1 = (rh_dist_meas_curr + lh_dist_meas_curr) / 2;
    u2 = (rh_speed_meas + lh_speed_meas) / 2;
    u3 = pitch_curr + PITCH_OFFSET + pitch_dir ;
    u4 = angular_speed;
  ...
  if(time_curr - print_time_prev >= INTERVAL_PRINT) {
    ...
    if (Serial.available()) {
      ...
      pitch_dir = constrain(input_int, -10, 10);
      Serial.print("Pitch Direction changed to ");
      Serial.println(pitch_dir);
      Serial.print("\n\r");
    }
    ...
    if(sbuf[sbuf_cnt]==0x0a)
       {
           ...
           result = 1;
           /*// Print angle values each time the sensor receives value.
           Serial.print("\n\r");
           for(i=0;i<number_of_item;i++) {  
             Serial.print(item[i]);  
             Serial.print(" "); 
