...
void loop() {
  ...
  if (time_curr - control_time_prev >= INTERVAL_CONTROL) {
    ...
    x1 = (rh_dist_meas_curr + lh_dist_meas_curr) / 2;
    x2 = (rh_speed_meas + lh_speed_meas) / 2;
    x3 = pitch_curr;
    x4 = angular_speed;
    ...
    voltage = -(k1*x1 + k2*x2 + k3*x3 + k4*x4);  // Something wrong
    if (voltage < 0) {
      voltage = -voltage;
      rh_dir = 1;
      lh_dir = 1 - rh_dir;
  ...
  if(time_curr - print_time_prev >= INTERVAL_PRINT) {
    ...
    */
    ...
    Serial.print(rh_pwm);   Serial.print(',');
    Serial.print(lh_pwm);   Serial.print(',');
    Serial.print(rh_dir);   Serial.print(',');
    Serial.print(lh_dir);   Serial.print('\n');
